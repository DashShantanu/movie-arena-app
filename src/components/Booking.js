...
import screen from '../Img/5ff3a50c.webp';
...
const Booking = () => {
    ...
    return (<div style={{ backgroundColor: 'white', color: 'black' }}>
        <Stack sx={{ margin: "30px 600px", padding: '10px', width: '400px', display: 'flex', alignItems: 'center' }} direction='row'>
            ...
            <Typography variant='h5'>{temp.title}</Typography>
            <Avatar
                src={`https://image.tmdb.org/t/p/original/${temp.backdrop_path}`}
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
            <h3>Prime : <CurrencyRupeeIcon fontSize='small' />600  Total Prime Count:   {c}</h3>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
            <h3>Classic : <CurrencyRupeeIcon fontSize='small' /> 450   <span> Total Classic Count:   {p}</span></h3>
        </div>
        ...
        <div className="container" style={{ display: 'flex', justifyContent: 'center' }}>
            <div>
                <img src={screen} alt='screen this way' style={{ width: '800px', marginLeft: '200px' }} />
            </div>

            <Link style={{ textDecoration: 'none' }} to={(600 * c + 450 * p) === 0 ? '' : '/checkout'} state={{ data, c, p, details }}>
                <Button disabled={(600 * c + 450 * p) === 0 ? true : false} variant='contained' color='secondary'>Book Tickets  ({600 * c + 450 * p})</Button>
            </Link>
